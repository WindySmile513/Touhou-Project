...
export async function content(config,pack){
	...
	lib.translate.anzhi="暗置";
	lib.translate.visible_mingzhi="明置";
	//明置卡牌相关函数
	lib.element.player.getMingzhiCards=function(){
		let func=() => true,position='h';
		for(let i=0;i<arguments.length;i++){
			if(typeof arguments[i]=='function') func=arguments[i];
			if(typeof arguments[i]=='string') position=arguments[i];
		}
		return this.getCards(position,card => 'ej'.includes(get.position(card))||(Array.isArray(card.gaintag)&&card.gaintag.includes('visible_mingzhi')&&func(card)));
	}
	lib.element.player.hasMingzhiCards=function(){
		return this.getMingzhiCards(...arguments).length>0;
	}
	lib.element.player.countMingzhiCards=function(){
		return this.getMingzhiCards(...arguments).length
	}
	lib.element.player.getAnzhiCards=function(){
		let func=() => true,position='h';
		for(let i=0;i<arguments.length;i++){
			if(typeof arguments[i]=='function') func=arguments[i];
			if(typeof arguments[i]=='string') position=arguments[i];
		}
		return this.getCards('h',card => (!Array.isArray(card.gaintag)||!card.gaintag.includes('visible_mingzhi'))&&func(card));
	}
	lib.element.player.hasAnzhiCards=function(){
		return this.getAnzhiCards(...arguments).length>0;
	}
	lib.element.player.countAnzhiCards=function(){
		return this.getAnzhiCards(...arguments).length
	}
	//卡牌明置
	...
	lib.skill._spell={
		...
		async content(event,trigger,player){
			if(player.node.fuka){
				let skillname,evt=trigger;
				while(evt.parent){
					evt=evt.getParent();
					if(evt.skill&&lib.skill[evt.skill].spell){
						skillname=evt.skill;
						break;
					}
				}
				if(skillname){
					...
					game.log(player,`开启符卡【${get.translation(player.storage.spell)}】符卡`);
					if(info.Fuka_init) await info.Fuka_init(player);
					player.addSkills(info.spell);
				...
				if(player.storage.spell){
					let info=lib.skill[player.storage.spell];
					if(info.Fuka_onremove) await info.Fuka_onremove(player);
					game.log(player,`的【${get.translation(player.storage.spell)}】符卡结束`);
					player.removeSkills(info.spell);
					player.addSkillTrigger(player.storage.spell);
					if(info.infinite) player.die();
	...
	lib.element.content.Fuka = function () {
		...
		if (!player.node.fuka) {
			...
			if (skillname) {
				...
				if (info.spell) {
					if (num != undefined) {
						let list = info.spell.slice(0);
						let n = list.findIndex(i => typeof i == 'object');
						let slist = list.splice(n, list.length - 1);
						player.addSkills(slist[num - 2]);
					}
					else {
						for (var i = 0; i < info.spell.length; i++) {
							if (typeof info.spell[i] == 'object') continue;
	...
	lib.element.player.Fuka = function (type) {
		var next = game.createEvent('Fuka');
		next.player = this;
		if (type) next.num = type;
	...
	/*形如
		mod:{
			liliCost:function(player,skill,cost){
				return cost-3;
			}
		},
	*/
	lib.element.player.getLiliCost = function (skill){
		let cost = typeof lib.skill[skill].liliCost == 'function' ? lib.skill[skill].liliCost(this) : lib.skill[skill].liliCost;
		return game.checkMod(this,skill,cost,'liliCost',this);
	}
	...
	lib.element.content.gainlili = function () {
		...
		if (player.hasSkillTag("liliLock")) return;
		if (player.node.fuka) {
	...
	if ((config.gezidedongfanglili || get.mode() == 'stg') && get.mode() != 'library') {
		...
		if (game.hasExtension("十周年UI")) lib.init.css(lib.assetURL + 'extension/东方project', 'decadeUIconfig');
	...
	/*-----------------------------------------技能牌-----------------------------------------*/
	//拥有技能牌
	lib.element.player.hasJinengpai = function (name) {
		if (name) return this.getJinengpai('j', name).length;
		return this.countJinengpai();
	}
	//技能牌数量
	lib.element.player.countJinengpai = function (name) {
		return this.getJinengpai(name).length;
	}
	//获取技能牌的信息
	lib.element.player.getJinengpai=function(name){
		let cards=Array.from(this.node.jinengpai?.childNodes||[]);
		if(name) cards=cards.filter(i => get.name(i)==name);
		return cards;
	}
	//添加技能牌
	...
	lib.element.player.removeJudgen=function(card){
		...
		if(get.itemtype(card) == 'card') next.cards = [card];
		else if(get.itemtype(card) == 'cards') next.cards = card;
		else return;
		next.player = this;
	...
	/*类皆杀技能管理*/
	//主要问题是有可能场上存在不止一个皆杀类技能，所以很麻烦
	//gezi_death_priority是类皆杀技能的优先级，皆杀本身默认为0，优先级高的技能会覆盖优先级低的
	//类皆杀技能主要是指会改变胜利条件，以及ai态度的，总之很麻烦
	//强行造一个映射表用来标记lib.onover数组里面各函数对应的优先级
	lib.onover_priority = new Map();
	lib.modAttitudeFunctions = new Map();
	//这个函数是负责制造用于塞进lib.onover数组的函数，以及负责修改modAttitude，顺带返回一个id用来标记这个函数
	get.trackedFunction = function(priority, func1, func2) {
		const id = Symbol(); //唯一标识符
		lib.onover_priority.set(id, priority);
		lib.modAttitudeFunctions.set(id, func2);
		let maxPriority = Math.max(...Array.from(lib.onover_priority.values())), entries = Array.from(lib.onover_priority.entries());
		let key = entries.filter(entry => entry[1] == maxPriority).lastItem[0];
		lib.modAttitudeFunctions.get(key)();
		const fc = function(){
			const myPriority = lib.onover_priority.get(id);
			const allPriority = Array.from(lib.onover_priority.values());
			//同一优先级的都会执行，理论上来讲会是后面的覆盖前面的
			if(Math.max(allPriority)==myPriority) func1();
		}
		fc._id = id;
		return [id,fc];
	}
	...
	lib.skill._enhance = {
		...
		check: function (event, player) {
			...
			var card = event.card;
			switch (card.name) {
				case 'gezi_danmakucraze': return (player.countCards('h') < player.hp) || player.countCards('h', { name: 'sha' }) || player.hp >= 2;
				case 'gezi_caifang': return player.lili > 2;
				case 'gezi_xuyuanshu': return player.lili > 1;
				case 'gezi_huazhi': return player.lili > 1;
	...
	lib.element.player.$effectn = function (name, frame, left, top) {
		if (game.getExtensionConfig("东方project", "dongfangtexiao")) {
			...
			Animation.style.backgroundSize = "100% 100%";
			var zhen = 0;
			var ID = setInterval(function () {
				...
				img.src = lib.assetURL + "extension/东方project/effect/xuliezhen/" + name + "/" + zhen + ".png";
	...
	lib.element.content.chosenToCompare = function () {
		...
		game.delay(2);
		game.asyncDraw([target, player].sortBySeat());
		"step 9";
			...
			scenes: [
				{
					...
					players: [
						...
						},
						{
							...
							maxlili: null,
							playercontrol: false,
							handcards: [["sha", "diamond", 10],],
							equips: [],
							judges: [],
						},
					],
					cardPileTop: [["juedou", "spade", 8], ["shunshou", "diamond", 4]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [],
				...
				},
				{
					...
					players: [
						...
						},
						{
							...
							maxlili: null,
							playercontrol: false,
							handcards: [["tao", "heart", 3]],
							equips: [["gezi_hourai", "spade", 8]],
							judges: [],
						},
					],
					cardPileTop: [["gezi_caifang", "spade", 8], ["juedou", "spade", 2], ["guohe", "heart", 6]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [],
				...
				},
				{
					...
					players: [
						...
						},
						{
							...
							maxlili: null,
							playercontrol: false,
							handcards: [["tao", "diamond", 12]],
							equips: [],
							judges: [],
						},
					],
					cardPileTop: [["juedou", "spade", 2], ["sha", "spade", 5], ["shan", "spade", 5], ["tao", "heart", 8]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [],
				...
				},
				{
					...
					players: [
						...
						},
						{
							...
							playercontrol: false,
							handcards: [],
							equips: [["gezi_mirror", "club", 8]],
							judges: [],
						},
					],
					cardPileTop: [["sha", "club", 3], ["sha", "club", 4], ["sha", "spade", 5]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [],
				...
				},
				{
					...
					players: [
						...
						},
						{
							...
							maxlili: null,
							playercontrol: false,
							handcards: [["wuzhong", "heart", 8]],
							equips: [],
							judges: [],
						},
					],
					cardPileTop: [["sha", "club", 3], ["gezi_zhiyuu", "club", 13]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [],
				...
				},
				{
					...
					cardPileTop: [["sha", "spade", 5], ["sha", "spade", 2]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [["gezi_lianji", null, null]],
					gameDraw: false,
					replacepile: true,
					turns: [1, "lose"],
				},
				{
					...
					players: [
						...
						},
						{
							...
							maxlili: null,
							playercontrol: false,
							handcards: [['tao', 'heart', 13]],
							equips: [],
							judges: [["gezi_shengdun", null, null]],
						},
					],
					cardPileTop: [["sha", "spade", 5], ["sha", "spade", 2]],
					cardPileBottom: [],
					...
					players: [
						{
							...
							maxlili: null,
							playercontrol: true,
							handcards: [["gezi_caifang", "spade", 8]],
							equips: [],
							judges: [],
						},
						{
							...
							maxlili: null,
							playercontrol: false,
							handcards: [["tao", "spade", 8]],
							equips: [],
							judges: [],
						},
					],
					cardPileTop: [["sha", "heart", 5, 1], ["sha", "spade", 2]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [["gezi_lianji", null, null]],
					...
					players: [
						{
							...
							maxlili: null,
							playercontrol: true,
							handcards: [['guohe', 'spade', '3']],
							equips: [['gezi_stone', 'spade', '7']],
							judges: [['gezi_lingyong', null, null]],
						},
						{
							name: "gezi_lunasa",
							name2: "none",
							...
							maxlili: null,
							playercontrol: false,
							handcards: [["shan", "heart", 5], ["shan", "diamond", 7], ["wuxie", "spade", 12]],
							equips: [["gezi_mirror", "club", 8]],
							judges: [],
						},
					],
					cardPileTop: [["shunshou", "diamond", 3], ["sha", "spade", 2], ['sha', 'spade', 9]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [['gezi_shengdun', null, null]],
					gameDraw: false,
					replacepile: true,
					turns: [1, "lose"],
				},
			...
			scenes: [
				{
					...
					players: [
						...
						},
						{
							...
							maxlili: null,
							playercontrol: false,
							handcards: [["shan", "diamond", 10],],
							equips: [],
							judges: [],
						},
					],
					cardPileTop: [["sha", "spade", 8], ["gezi_zuiye", "club", 3]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [],
				...
				},
				{
					...
					cardPileTop: [["sha", "spade", 8], ["gezi_zuiye", "club", 3], ['sha', 'club', '7']],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [['gezi_shenyou', null, null]],
					skillPileBottom: [],
					gameDraw: false,
					replacepile: true,
					turns: [1, "lose"],
				},
				{
					...
					players: [
						...
						},
						{
							...
							maxlili: null,
							playercontrol: false,
							handcards: [["tao", "heart", 6], ["tao", "heart", 7], ["tao", "heart", 8], ["tao", "heart", 12]],
							equips: [],
							judges: [],
						},
					],
					cardPileTop: [["sha", "spade", 8], ["gezi_danmakucraze", "spade", 12], ['sha', 'club', 7]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [],
					...
					players: [
						{
							...
							maxlili: null,
							playercontrol: true,
							handcards: [["shan", 'diamond', 10], ["shan", 'diamond', 11], ["shan", 'heart', 2]],
							equips: [],
							judges: [],
						},
						{
							...
							maxlili: null,
							playercontrol: false,
							handcards: [["shan", "diamond", 4], ["shan", "diamond", 7], ["shan", "heart", 13]],
							equips: [],
							judges: [['gezi_shenyou', null, null]],
						},
					],
					cardPileTop: [["juedou", "heart", 11], ['guohe', 'club', 4]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [],
					...
					players: [
						{
							...
							maxlili: null,
							playercontrol: true,
							handcards: [['gezi_houraiyuzhi', 'heart', 12]],
							equips: [],
							judges: [],
						},
						{
							...
							maxlili: null,
							playercontrol: false,
							handcards: [["shan", "heart", 13]],
							equips: [],
							judges: [],
						},
					],
					cardPileTop: [["tao", "heart", 6], ["tao", 'heart', 4], ["tao", "heart", "9"], ['shan', 'diamond', 3]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [],
					...
					players: [
						{
							...
							maxlili: null,
							playercontrol: true,
							handcards: [['gezi_lingbi', 'spade', 4], ["wuxie", "diamond", 12], ['sha', 'spade', 9]],
							equips: [],
							judges: [],
						},
						{
							...
							maxlili: null,
							playercontrol: false,
							handcards: [["wuxie", "club", 12], ['shan', 'diamond', 5], ['tao', 'heart', 7]],
							equips: [],
							judges: [],
						},
					],
					cardPileTop: [["gezi_tianguo", "heart", 8], ['juedou', 'club', 1], ['shan', 'heart', 7], ['shan', 'diamond', 9], ["wuxie", "club", 12], ['shan', 'diamond', 5], ['tao', 'heart', 7]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [],
			...
			scenes: [
				{
					...
					players: [
						...
						},
						{
							...
							playercontrol: false,
							handcards: [],
							equips: [["gezi_yinyangyu", "heart", 5, -1], ["gezi_bailou", "club", 6, -1]],
							judges: [],
						},
					],
					cardPileTop: [["juedou", "heart", 11], ["tao", "diamond", 12], ["guohe", "club", 4], ["gezi_gungnir", "heart", 13], ["gezi_simen", "spade", 2], ["gezi_reidaisai", "heart", 6]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [],
					...
					players: [
						{
							...
							maxlili: null,
							playercontrol: true,
							handcards: [["sha", "diamond", 7, 1], ["gezi_deathfan", "heart", 1], ["tao", "heart", 8]],
							equips: [],
							judges: [],
						},
						{
							...
							maxlili: null,
							playercontrol: false,
							handcards: [["shan", "diamond", 10], ["tao", "heart", 12]],
							equips: [],
							judges: [],
						},
					],
					cardPileTop: [["shan", "diamond", 4], ["tao", "heart", 3]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [],
					skillPileBottom: [],
				...
				/*
				{
					...
					players:[
						...
						},
						{
							...
							maxlili:null,
							playercontrol:false,
							handcards:[["tao","heart",6],["tao", "heart", 7], ["tao", "heart", 8]],
							equips:[],
							judges:[],
						},
					],
					cardPileTop:[["sha","spade",8], ["danmakucraze","spade",12], ['sha', 'club', '7']],
					cardPileBottom:[],
					discardPile:[],
					skillPileTop:[],
					skillPileBottom:[],
				...
				*/
				{
					...
					players: [
						{
							...
							maxlili: null,
							playercontrol: true,
							handcards: [["juedou", "diamond", 1], ["gezi_zuiye", "club", 3]],
							equips: [["gezi_laevatein", "heart", 13]],
							judges: [["gezi_shenyou", null, 0]],
						},
						{
							name: "gezi_mokou",
							name2: "none",
					...
					cardPileTop: [["huazhi", "diamond", 9], ["sha", "club", 4], ['simen', 'spade', 2], ["sha", "club", 7], ['shan', 'diamond', 3], ['juedou', 'spade', 1]],
					cardPileBottom: [],
					discardPile: [],
					skillPileTop: [['gezi_lingyong', null, 0]],
					skillPileBottom: [],
					gameDraw: false,
					replacepile: true,
					turns: [1, "lose"],
	...
	//神隐
	//进入神隐状态，eternal为true的话不会自行结束，不为true的话会在每个回合结束时解除
	lib.element.player.shenyin=function(eternal){
		var next=game.createEvent('shenyin');
		next.player=this;
		next.eternal=eternal;
		next.setContent('shenyin');
		next._args=Array.from(arguments);
		return next;
	}
	lib.element.content.shenyin=function(){
		"step 0"
		event.trigger('shenyinBefore');
		"step 1"
		if(!event.eternal) player.teshuchuwai_End=true;
		"step 2"
		player.addSkill('teshuchuwai');
		"step 3"
		event.trigger('shenyinAfter');
	}
	...
	lib.skill._teshuchuwai_End={
		...
		content:function(){
			for(var i=0;i<game.players.length;i++){
				if(game.players[i].hasSkill('teshuchuwai')&&game.players[i].teshuchuwai_End){
					game.players[i].removeSkill('teshuchuwai');
					delete game.players[i].teshuchuwai_End;
				}
			}
		},
	}
	...
	lib.characterSubstitute['gezi_FSakura'] = [
		['gezi_FSakuraawake', [((lib.device || lib.node) ? 'ext:' : 'db:extension-') + '东方project/gezi_FSakuraawake.jpg']],
	]
	lib.characterSubstitute['gezi_shana'] = [
		['gezi_shana_awake', [((lib.device || lib.node) ? 'ext:' : 'db:extension-') + '东方project/gezi_shana_awake.jpg']],
	]
	lib.characterSubstitute['gezi_saber'] = [
		['gezi_saberAlter', [((lib.device || lib.node) ? 'ext:' : 'db:extension-') + '东方project/gezi_saberAlter.jpg','die:ext:东方project/gezi_saberAlter.mp3']],
	]
	lib.characterSubstitute['niuzhanshi'] = [
		['mordred', [((lib.device || lib.node) ? 'ext:' : 'db:extension-') + '东方project/mordred.jpg']],
	]
	lib.characterSubstitute['gezi_hinata'] = [
		['gezi_hinata_2', [((lib.device || lib.node) ? 'ext:' : 'db:extension-') + '东方project/gezi_hinata_2.jpg']],
		['gezi_hinata_4', [((lib.device || lib.node) ? 'ext:' : 'db:extension-') + '东方project/gezi_hinata_4.jpg']],
	]
	if (!lib.dynamicTranslate) lib.dynamicTranslate = {};
	lib.dynamicTranslate['gezi_shengguang'] = function (player) {
		return '你使用带有伤害标签的牌指定目标后，或成为带有伤害标签牌的目标后，可以与目标/来源拼点；' + (player.storage.gezi_saberAlter ? '赢的角色收回拼点牌' : '赢的角色摸一张牌') + '；你拼点时，可以消耗1点灵力，' + (player.storage.gezi_saberAlter ? '令对方拼点牌点数减半(向上取整)' : '令拼点牌点数翻倍') + '。';
	}
	lib.dynamicTranslate['gezi_fengchui'] = function (player) {
		//失落的白垩坚城 若你赢，你获得一点护甲
		//尘封的圆桌誓言 将你的拼点牌放置在牌堆底，摸一张牌
		//十五个世纪的追寻 你贴上一张连击，对方贴上一张灵涌
		//十四个冬日的奇遇 随机一张禁忌牌
		//于抵达理想的黎明 若你赢，获得两张拼点牌
		//守护命运长夜 若你没赢，从牌堆获得一张对方拼点牌同类型的牌
		if (!player.storage.gezi_xingtu) return '你使用带有伤害标签的牌指定目标后，可以与目标拼点。';
		var str = {
			'lostcamelot': '若你赢，你获得一点护甲',
			'sealvow': '将你的拼点牌放置在牌堆底，摸一张牌',
			'longfind': '你贴上一张连击，对方贴上一张灵涌',
			'wondermiss': '随机获得一张禁忌牌',
			'daybreak': '若你赢，获得两张拼点牌',
			'fsn': '若你没赢，从牌堆获得一张对方拼点牌同类型的牌',
		};
		return '你使用带有伤害标签的牌指定目标后，可以与目标拼点。然后' + str[player.storage.gezi_xingtu] + '。';
	}
	// attack:'攻击',
	// defense:'防御',
	// support:'支援',
	// disrupt:'控场',
	lib.cwattack = ['sha', 'juedou'];
	lib.cwdefense = ['shan', 'wuxie'];
	lib.cwsupport = ['tao', 'gezi_danmakucraze', 'wuzhong', 'gezi_reidaisai', 'gezi_reidaisai2'];
	lib.cwdisrupt = ['guohe', 'shunshou', 'gezi_caifang'];
	//一个车万下的子类型判断
	//包含，返回车万对应的子类型，不包含，返回三国杀下的子类型
	get.cwtype = function (card) {
		let dfp = ['sha', 'juedou', 'shan', 'wuxie', 'tao', 'gezi_danmakucraze', 'wuzhong', 'gezi_reidaisai', 'gezi_reidaisai2', 'guohe', 'shunshou', 'gezi_caifang'];
		if (dfp.includes(card.name)) {
			for (var i of ['attack', 'defense', 'support', 'disrupt']) if (lib['cw' + i].includes(card.name)) return i;
		}
		else return get.subtype(card);
	}
	//瞬发寄
	lib.element.player.dfpjShunfajiInit = function (skillname, popname) {
		if (!this.isUnderControl(true)) {
			return;
		}
		if (!popname) popname = true;
		var info = lib.skill[skillname];
		if (!info) return;
		if (info.clickable) {
			var button = ui.create.div('.dfpj-shunfaanniu', this);
			if (popname == false) button.innerHTML = get.translation(skillname);
			var player = this;
			button.listen(function () {
				if (player.hasSkill(skillname, true, true, false)) {
					if (info.clickable) {
						if ((info.clickableFilter && !info.clickableFilter(player)) || !player.hasSkill(skillname, false, true, true)) {
							alert("当前不可发动！");
							return;
						}
	...
	var list = {
		...
		gezi_saberxt: '星铁·阿尔托莉雅',
		gezi_saberxt_ab: '阿尔托莉雅',
		gezi_shigure: '时雨',
		gezi_tsubaki: "椿",
		gezi_yudachi: '夕立',
		gezi_aliceWLD: "爱丽丝",
		...
		gezi_aqua: '阿库娅',
		gezi_miku: '初音',
		gezi_sento_isuzu: '五十铃',
		gezi_kirihime_yoruka: '夜架',
		gezi_eucliwood: '优克莉伍德',//_hellscythe·海尔赛兹
		gezi_jolyne: '徐伦',
		stone_free: '石之自由',
		gezi_shana: '夏娜',
		gezi_michele: '米雪儿',
		gezi_gilgamesh: '吉尔伽美什',
		gezi_nero_claus: '圣诞尼禄',
		gezi_jeanne: '贞德',
		gezi_helena: '海伦娜',
		gezi_Mysterious_Heroine_X: '谜之女主角Ｘ',
		gezi_unknown_eks: '谜之女主角X',
		gezi_sb_zhouyu: '周瑜',
		gezi_chenglian: '小怜',
		gezi_ironmaiden: '梅登贞德',
		gezi_lanran: '蓝染',
		gezi_hinata: '雏田',
		gezi_giriyu: '雾夕',
		gezi_Yorihime: '依姬',
